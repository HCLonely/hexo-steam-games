...
function generatePage(source_dir, globalConfig, root, config = {}) {
    let games = [];
    if (!fs.existsSync(path.join(source_dir, `/_data/${config.id || globalConfig.steam.steamId}.json`))) {
        log.info(`Can't find steam game data, please use 'hexo steam -u' command to get data`);
    } else {
        games = JSON.parse(fs.readFileSync(path.join(source_dir, `/_data/${config.id || globalConfig.steam.steamId}.json`))).slice(0, config.length || globalConfig.length || 1000);
        log.info(games.length + ' games have been loaded');
    }

    let __ = i18n.__(globalConfig.language);
    let theme = config.theme || globalConfig.steam.theme;
...
module.exports = async function (locals) {
    ...
    if (config.steam.steamId) {
        return generatePage(this.source_dir, config, root);
    }
    if (config.steam.steamInfos) {
        return config.steam.steamInfos.map((steamInfo) => generatePage(this.source_dir, config, root, steamInfo));
    }
